...
const DEFAULT_FEE_IN_SATOSHI = 250;
const BTC_TO_SAT = 1.0 / 100000000;
...
export class BtcProvider {
  ...
  async getBalance(from: Wallet): Promise<bigint> {
    ...
    const btcAmount = availableSatoshis * BTC_TO_SAT;
    return btcAmount;
  }
...
export class BitcoinProvider extends SiteProvider<BtcProvider> {
  ...
  async sendTransaction(req: SendTransactionRequest): Promise<SignedTransaction> {
    ...
    const satoshisToSend = Math.round(parseFloat(req.txValue.trim()) / BTC_TO_SAT);
    try {
      let hash = await this.provider.transfer(
        req.from,
        toAddr,
        satoshisToSend,
        DEFAULT_FEE_IN_SATOSHI
      );
      return {
        chain: this.chain,
  ...
  public async fetchGasFee(_fromAddr: string): Promise<bigint> {
    ...
    return DEFAULT_FEE_IN_SATOSHI * BTC_TO_SAT;
  }
