...
import { login, logout, loginWithToken } from "./setup";
import {
  CubeSigner,
...
describe("session (with user input token)", () => {
  ...
  beforeEach(async () => {
    ...
    token = btoa(JSON.stringify(await storage.retrieve()));
...
describe("session using token param", () => {
  ...
  beforeEach(async () => {
    ...
    const storage = new MemorySessionStorage<SignerSessionData>();
    session = await role.createSession(storage, "snap-sign-test");
    const sessionData: SignerSessionData = await storage.retrieve();
