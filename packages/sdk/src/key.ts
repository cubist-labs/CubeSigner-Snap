...
export class Key {
  ...
  private async update(request: UpdateKeyRequest): Promise<KeyInfo> {
    const resp = await this.#cs.management().patch("/v0/org/{org_id}/keys/{key_id}", {
  ...
  private async fetch(): Promise<KeyInfo> {
    const resp = await this.#cs.management().get("/v0/org/{org_id}/keys/{key_id}", {
      params: { path: { org_id: this.orgId, key_id: this.id } },
      parseAs: "json",
    });
    const data = assertOk(resp);
   ...
   * @internal
   * */
  static async listKeys(cs: CubeSigner, orgId: string, keyType?: KeyType): Promise<Key[]> {
